...
Order::Order(string name, unsigned int quant, unsigned int price) {
	productName = name;
	quantity = quant;
	this->price = price;
}
...
void Table::removeAllOrders() {
	while (tableOrders != NULL)
	{
		tableOrders->~Order();
	}
	tableOrders = NULL;
...
MyRestaurant::MyRestaurant(int tableAmount, Product* productList) {
